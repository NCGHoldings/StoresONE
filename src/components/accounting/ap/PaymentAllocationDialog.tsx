...
import { useCreatePaymentAllocation, usePaymentAllocations, VendorPayment } from '@/hooks/useVendorPayments';
...
export default function PaymentAllocationDialog({ open, onOpenChange, payment }: PaymentAllocationDialogProps) {
  const { data: invoices } = useInvoices();
  const { data: existingAllocations } = usePaymentAllocations(payment?.id || null);
  ...
  const allocatedAmount = existingAllocations?.reduce((sum, a) => sum + Number(a.amount), 0) || 0;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Allocate Payment {payment.payment_number}</DialogTitle>
        </DialogHeader>
